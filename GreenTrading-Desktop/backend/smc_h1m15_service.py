...
# -*- coding: utf-8 -*-
"""
GreenTrading Desktop - SMC H1 + M15 PRO Service (FASE 3A)

Servicio de análisis para la estrategia SMC_H1_M15_PRO.
Completamente separado de smc_m15_service.py — zero contaminación.

Responsabilidades:
  - Orquestar el engine SMC_H1_M15_PRO
  - Smart sync / debounce hacia Supabase con strategy_id = 'SMC_H1_M15_PRO'
  - Cache de estado por símbolo (separada de la cache de SMC_M15_PRO)
"""
...
print("SMC_H1M15_SERVICE_PATH:", __file__)
...
def _has_relevant_changes_h1m15(symbol: str, new_data: dict) -> bool:
    """
    ...
    """
    if symbol not in _setup_cache_h1m15:
        _setup_cache_h1m15[symbol] = new_data
        return True

    old_data = _setup_cache_h1m15[symbol]

    critical_fields = ["estado", "entrada", "stoploss", "tp_1_1", "score", "zona_desde", "zona_hasta"]
    for field in critical_fields:
        if old_data.get(field) != new_data.get(field):
            print(f"H1M15 SYNC TRIGGER: {symbol} - {field} cambiado de {old_data.get(field)} a {new_data.get(field)}")
            _setup_cache_h1m15[symbol] = new_data
            return True
    ...
    if old_price and old_price > 0:
        ...
        if price_change_pct > 1.0:
            print(f"H1M15 SYNC TRIGGER: {symbol} - precio cambió {price_change_pct:.2f}%")
            _setup_cache_h1m15[symbol] = new_data
            return True
...
def sync_setup_to_supabase_h1m15(analysis_result: dict) -> None:
    """
    Sincroniza setup SMC_H1_M15_PRO con Supabase solo si hay cambios relevantes.

    Implementa:
    - Smart sync / debounce (evita spam updates)
    - Duplicate closed zone guard (por strategy_id='SMC_H1_M15_PRO')
    - NO toca registros de SMC_M15_PRO
    """
    if not supabase_service:
        print("  H1M15 SUPABASE SYNC: Service not available")
        return

    estado_actual = analysis_result.get("estado", "")
    if estado_actual in ("SIN SETUP", "SIN_SETUP"):
        print(f"  H1M15 SUPABASE SYNC: Skipping {analysis_result.get('symbol')} - SIN_SETUP")
        return

    if not analysis_result.get("entrada") or not analysis_result.get("stoploss"):
        print(f"  H1M15 SUPABASE SYNC: Skipping {analysis_result.get('symbol')} - falta entrada o stoploss")
        return
    ...
    if not _has_relevant_changes_h1m15(symbol, critical_data):
        print(f"  H1M15 SUPABASE SYNC: Skipping {symbol} - no hay cambios relevantes")
        return

    print(f"  H1M15 SUPABASE SYNC: Preparando sync para {symbol}")
    ...
    if existing:
        ...
        print(f"  H1M15 SUPABASE SYNC: UPDATE id={setup_id}, estado={setup_data['estado']}")
        result = supabase_service.update_setup(setup_id, updates)
        if result:
            print(f"H1M15 SUPABASE SYNC: Updated {symbol}")
        else:
            print(f"H1M15 SUPABASE SYNC WARNING: update devolvió None para {symbol}")
    else:
        ...
        print(f"\nDUPLICATE_CLOSED_ZONE_CHECK (H1M15):")
        ...
        if closed_setup:
            # Zona ya cerrada — resetear display a SIN_SETUP para evitar recrear
            ...
            print(f"  H1M15 SUPABASE SYNC: SKIP create_setup — zona ya cerrada (TP/SL)")
            return

        result = supabase_service.create_setup(setup_data)
        if result:
            print(f"H1M15 SUPABASE SYNC: Created setup for {symbol}, id={result.get('id')}")
        else:
            print(f"H1M15 SUPABASE SYNC WARNING: create_setup devolvió None para {symbol}")
...
def analyze_symbol_smc_h1m15(
    ...
    """
    Fachada pública para análisis SMC_H1_M15_PRO.

    Flujo:
    1. Delegar al engine SMCH1M15ProEngine
    2. Si hay zona válida: sincronizar con Supabase (strategy_id='SMC_H1_M15_PRO')
    ...
    Args:
        ...
        df_m1: M1 candles (NO se usa — aceptado para compatibilidad)
    ...
    result = engine.analyze(
        ...
        create_sin_setup_response=create_sin_setup_h1m15_response,
    ...
    if (
        ...
        sync_setup_to_supabase_h1m15(result)

    return result


def print_result_summary(result: dict) -> None:
    """Print formatted result summary for SMC_H1_M15_PRO."""
    print(f"\n  H1M15 RESULT SUMMARY:")
    print(f"     Symbol: {result.get('symbol')}")
    print(f"     Tendencia H1: {result.get('tendencia_h1')}")
    print(f"     Tendencia M15: {result.get('tendencia_m15')}")
    print(f"     Ultimo Evento M15: {result.get('ultimo_evento_m15')}")
    zona = result.get("zona_madre_m15", {})
    print(f"     Zona: {zona.get('desde', 0):.2f} - {zona.get('hasta', 0):.2f}")
    print(f"     Score: {result.get('score')}")
    print(f"     TP Ratio: {result.get('tp_ratio', TP_RATIO)}")
    print(f"     OB: {result.get('ob')}, FVG: {result.get('fvg')}, Barrida: {result.get('barrida')}")
    print(f"     Estado: {result.get('estado')}")
    print(f"     Estado H1M15: {result.get('estado_h1_m15')}")
    print(f"{'='*60}\n")
